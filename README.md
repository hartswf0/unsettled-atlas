...
The model is an instrument of the triangle rather than a textbox hidden in the council. It is called through the Responses API — instructions separate from input, structured output by strict JSON Schema, reasoning effort where the model takes it, no temperature at all — and the model list comes from `/v1/models` on your own account rather than a name baked into the source, so a new generation appears without a source edit. It is given the compiled world state, `icosa-context-v2`: the address, what contains it and what it touches, the ground, the council with its topic topology and who answers whom and where every seat's name came from, what has been said and carried and escalated, and the evidence with its provenance. The pipeline is triangle → retrieve → assemble context → reason → structured proposal → a human accepts → record, and no server-side conversation memory is used, because the memory should be recompiled from the local record every call. The panel shows what the model is allowed to know before anything is asked, and prints the whole object on request. Your key is held for one browser tab, never written to the record, and scrubbed from every export — it used to live on the record and leave inside it, which is the one path the record exists for.
