...
import { inferGraphStatements } from './infer-graph.mjs';
...
async function run(){
 ...
 const ctx={lat,lon,radiusKm,depth,sinceDays,limit};const selected=String(a.sources||defaultSourcesFor(ctx).join(',')).split(',').map(s=>s.trim()).filter(Boolean);for(const id of selected)if(!ADAPTERS[id])throw new Error(`no executable adapter: ${id}`);
 ...
 const sourceRuns=[];const batches=await Promise.all(selected.map(async id=>{const started=new Date().toISOString();try{const signals=await ADAPTERS[id](ctx);sourceRuns.push({id,status:'ok',started,finished:new Date().toISOString(),count:signals.length});return signals;}catch(err){sourceRuns.push({id,status:'error',started,finished:new Date().toISOString(),count:0,error:String(err?.message||err)});return[];}}));
 const first=batches.flat();let enriched=[];const es=new Date().toISOString();try{enriched=await enrichGLEIF(first,{gleifLimit:Math.min(50,Number(a['gleif-limit']||30))});sourceRuns.push({id:'gleif-enrichment',status:'ok',started:es,finished:new Date().toISOString(),count:enriched.length});}catch(err){sourceRuns.push({id:'gleif-enrichment',status:'error',started:es,finished:new Date().toISOString(),count:0,error:String(err?.message||err)});}
 const addressor=createAddressor(),{good:signals,rejected}=addressSignals([...first,...enriched],addressor,depth);signals.sort((a,b)=>String(a.atlas_address).localeCompare(String(b.atlas_address))||a.source.localeCompare(b.source)||a.id.localeCompare(b.id));
 const graph=buildGraph(signals),statements=[...inferStatements(signals),...inferGraphStatements(graph)],cells=groupCells(signals);
